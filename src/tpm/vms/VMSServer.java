...
public class VMSServer implements Runnable {
	...
	public VMSServer(int port, String configPath) throws IOException {
		this.serverPort = port;
		this.sslServerSocket = null;
		this.threadPool = Executors.newFixedThreadPool(NUM_THREADS);
		this.config = new VMSConfiguration(configPath);
	...
	public void run() {
		...
		for (;;) {
			...
			this.threadPool.execute(new VMSService(config, clientSocket, nounces));
		}
